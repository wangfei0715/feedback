						...
						$.ajax({
							...
							success: function(s){
								...
								if(s=="y"){
									...
									errorobj=inputobj;
									// 20110703 改动 s -> 该用户名已被使用
									var errmsg = "输入错误";
									var name = $(inputobj)[0].name;
									if(name == "user.loginname"){errmsg = "该用户名已被使用";}
									else if(name == "oldpassword"){errmsg = "输入原密码不正确";}
									$.fn.Validform.sn.showmsg(errmsg,settings.tiptype,{obj:inputobj});
								}
							}
						});
					}else{
		...
		regcheck:function(type,gets){
			switch(type){
				...
				case "s":
					return isNaN(gets);
				case "s5-18":
					// 20110703 改动 s6-18 -> s5-18
					var repost= /^[\u4E00-\u9FA5\uf900-\ufa2d\w]{5,18}$/;
					return repost.test(gets);
				case "p":
					var repost= /^[0-9]{6}$/;
					return repost.test(gets);
